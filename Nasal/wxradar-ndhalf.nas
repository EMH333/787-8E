...
var sqr = func(n) return n * n;
	...
	for (var n = 0; n < 8; n += 1)
	{
		...
		if (getprop(storm ~ "latitude-deg") != nil) 
		{
			...
			var storm_precip = getprop(storm ~ "reflectivity-norm");

			var storm_size = getprop(storm ~ "radius-nm");
			...
			if ((Deflection(bearing_deg, 60) != 60) and (Deflection(bearing_deg,60) != -60) and (distance_nm <= range_nm) and (alt_visible == 1))
			{
				...
				# Storm size on display

				## 1 nm = 20px WIDTH and 40px HEIGHT

				setprop(storm ~ "width-px", storm_size * 20);
				setprop(storm ~ "height-px", storm_size * 40);
